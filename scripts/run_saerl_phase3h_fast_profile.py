"""Run a faster 3-family SAERL sweep with isolated artifacts.

This profile is meant for faster iteration than the locked Phase-3h runner:
- fewer scenarios per family
- fewer episodes per scenario
- capped adaptive horizons
- chemistry-aware SAERL anchor
- optional exclusion of known slow pathological cases
"""
...
DEFAULT_TAG = "fast_profile_20260302"
DEFAULT_EXCLUDE_CASES = "matr/2018-09-10_oed_3_CH7"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run a faster isolated Phase-3h 3-family SAERL sweep.")
    ...
    p.add_argument("--exclude-dataset-cases", type=str, default=DEFAULT_EXCLUDE_CASES)
    p.add_argument("--max-files-per-dataset", type=int, default=2)
    p.add_argument("--episodes-per-setting", type=int, default=1)
    p.add_argument("--n-folds", type=int, default=2)
    p.add_argument("--max-steps", type=int, default=160)
    p.add_argument("--online-max-steps", type=int, default=220)
    p.add_argument("--min-episode-minutes", type=float, default=120.0)
    p.add_argument("--feasible-time-slack", type=float, default=1.35)
    p.add_argument("--max-steps-cap", type=int, default=3000)
    p.add_argument(
    ...
    p.add_argument("--cem-label-interval", type=int, default=32)
    return p.parse_args()
...
def main() -> None:
    ...
    families = [x.strip().lower() for x in str(args.dataset_families).split(",") if x.strip()]
    ...
    print(f"Completed fast profile tag: {args.tag}", flush=True)
